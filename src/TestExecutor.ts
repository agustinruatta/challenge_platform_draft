...
export default class TestExecutor {
    ...
    async executeTest(
        ...
        const executionInfo = await this.executeDockerCall(techStack, fileName, exerciseId);
    ...
    private createTmpFile(techStack: TECH_STACKS, userAssessmentCode: string): string {
        ...
        const assessmentCodeWithPrefix = this.TECH_STACK_CONFIGS[techStack].userSubmittedCodePrefix + userAssessmentCode;

        //TODO: in a real environment I'd check if string is too long, raise an error in order to avoid fill disk with crap
        fs.writeFileSync('./user_assessments/' + fileName, assessmentCodeWithPrefix);
     ...
     * @param techStack
     */
    private getFileNamePathToTest(techStack: TECH_STACKS): string {
        return this.TECH_STACK_CONFIGS[techStack].fileNamePathToTest;
    }
    ...
    private parserIdAsNumber(techStack: TECH_STACKS, exerciseId: string): number {
        ...
        if (isNaN(number) || number < 1 || number > this.TECH_STACK_CONFIGS[techStack].amountOfExercises) {
            throw new Error('Invalid exercise ID');
        }
